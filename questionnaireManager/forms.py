...
class questionnaireForm(forms.ModelForm):
	class Meta:
		model = Questions
		fields = ('zip', 'animal', 'rooms', 'trait')
		widgets = {
                    'zip': forms.TextInput(attrs={'placeholder': 'Zip Code'}),
                    'animal': forms.TextInput(attrs={'placeholder': 'Are you looking for a cat or dog?'}),
                    'rooms': forms.TextInput(attrs={'placeholder': 'How many rooms are there where you live?'}),
                    'trait': forms.TextInput(attrs={'placeholder': 'Check the following that best describes the kind of pet you would own:'}),
			    }
